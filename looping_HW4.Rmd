...
library(ggpubr)
```
# Other examples where you might need a *for* loop
...
Task 2: Compare results using a function to compute yield (I've provided it for you) 
...
Just read through this (preview) and try to understand - We will go through in class!
...
speeds = rnorm(mean=mean_speed, sd=10*mean_speed, nsample)
summary(speeds)
...
# next car (row 4)
powerC = autopower(V=speeds, A = possible_cars$area[4], m=possible_cars$mass[4])
# lets look at what we get
summary(powerD)
