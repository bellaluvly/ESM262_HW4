...
### Note, I edited this RMD with the required changes for HW #4 ### 
...
# do conversion from km to meters per second
speed_base = 80 * 0.28
...
they are travelling at a range of speeds - mean is 80km/hr and speeds tend to
be log-normally distributed)

How would the total power consumed vary by car
So if all cars are car A; OR all cars are car B; OR all cars are car C; OR all cars are car D

We will use *sample* here to generate speeds for our 200 cars and look at different ways to
...
# assume a log normal distribution of speeds with mean 100km/hr, and standard deviation that is 10 km/min -> convert into m/s (using *0.277)
...
speeds = rlnorm(mean=mean_speed, sd=log(10*0.277), nsample)
summary(speeds)
...
# aka: needing to put in individual car
...
# note: powerallr1 has 800 values in column, 200 observations for each car 
# > (nsample = 200)
...
# the first part, generating speeds is the same
# proceed with estimating average power use given  each car
